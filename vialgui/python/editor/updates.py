...
2.  Click Update now. On recent firmware the keyboard restarts into update mode by itself; on older firmware the dialog asks you to double-tap the reset button on the half that has the USB cable (its LED blinks). Then choose Write to RPI-RP2 drive: in the folder dialog select the drive named RPI-RP2 itself and click Select Folder (the firmware is already loaded, needs no driver). Or use Select keyboard (USB). Wait for the write to finish — the keyboard restarts by itself.
