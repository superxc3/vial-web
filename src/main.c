...
static PyObject* vialglue_download_file(PyObject *self, PyObject *args) {
    // Ask the page to save a file that is served next to it (firmware/*.uf2); see index.html
    const char *path;
    ...
    EM_ASM({
        postMessage({cmd: "download_file", path: UTF8ToString($0)});
    }, path);
